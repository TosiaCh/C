/* Antonina Chwała 2023.01.20 email: tosia.chwala@studen.uj.edu.pl
instrukcja kompilacji: gcc nbody.c -o nbody
instrukcja uruchomienia: ./nbody Plummer5.dat */
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
#define eps 0.0
...
int main(int argc, char *argv[]) {
  ...
  if (argc >= 2) {
    ...
    if (datafile == NULL) {
      printf("ERROR: nie udalo sie otworzyć pliku.\n");
      exit(-1);
    }
  } else {
  ...
  for (int i = 0; i < num; i++) {
    fscanf(datafile, "%lf %lf %lf %lf %lf %lf %lf", &liczby[0], &liczby[1],
           &liczby[2], &liczby[3], &liczby[4], &liczby[5], &liczby[6]);
